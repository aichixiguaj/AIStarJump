...
import com.aichixiguaj.aisports.data.TorsoAndBodyDistance
...
class MoveNet(
    ...
    companion object {
        ...
        fun create(
            ...
            // 加载模型
            val modelTypeName = if (modelType == ModelType.Lightning) {
                LIGHTNING_FILENAME
            } else {
                THUNDER_FILENAME
            }

            val modelFile = FileUtil.loadMappedFile(contextWR.get()!!, modelTypeName)
            // 初始化
            return MoveNet(
        ...
        fun create(contextWR: WeakReference<Context>, device: Device): MoveNet =
            create(contextWR, device, ModelType.Lightning)
    }
        ...
        cropRegion?.run {
            ...
            inputTensor?.let { input ->
                ...
                for (idx in 0 until numKeyPoints) {
                    ...
                    keyPoints.add(
                        KeyPoint(BodyPart.fromInt(idx), PointF(x, y), score)
                    )
                    totalScore += score
                }
            }
